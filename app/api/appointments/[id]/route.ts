...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    const doc = await adminDb.collection("appointments").doc(params.id).get();
    if (!doc.exists) {
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    await adminDb.collection("appointments").doc(params.id).update(updateData);
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    const now = new Date();
    await adminDb.collection("appointments").doc(params.id).update({ status: "cancelled", updatedAt: now });
