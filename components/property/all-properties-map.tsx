...
export default function AllPropertiesMap({ onSelect }: { onSelect?: (property: Property | null) => void } = {}) {
  ...
  const points = properties.filter(p => p.gps && typeof p.gps.lat === "number" && typeof p.gps.lng === "number");
  console.log("[AllPropertiesMap] points (có gps):", points); // log các property có gps
  const center: [number, number] = points.length > 0 && points[0].gps
            ...
            p.gps ? (
              ...
              >
                <Popup>
                  ...
                  <div className="text-blue-700 font-semibold">{p.price?.toLocaleString()} đ</div>
                </Popup>
              </Marker>
            ) : null
          )}
